...
const App: React.FC = () => {
  ...
  const [b, setB] = useState<number>(0); // bias state starting at 0
  /**
   * Make network call
   * Method: POST
  ...
  const fetchPrediction = async (slope: number, intercept: number) => {
    try {
      const response = await fetch('http://localhost:8000/calculate', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ k: slope, b: intercept }), //send both parameters
      });
  ...
  // Update the useEffect to watch both k and b
  // slider changes -> state k, b changes -> triggers re-render, useEffect: run the fetchPrediction effect whenever k, b changes -> fetchPrediction: API call
  useEffect(() => {
    fetchPrediction(k, b);
  }, [k, b]);
  ...
  const handleBiasChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const val = parseFloat(e.target.value);
    setB(isNaN(val) ? 0:val);
  };

  return (
    <div style={{ padding: '40px', fontFamily: 'system-ui, sans-serif' }}>
      <h2>Interactive Slope and Intercept Demo</h2>
      ...
      {/* New Bias Slider */}
      <div style={{ marginBottom: '20px', background: '#f4f4f4', padding: '15px', borderRadius: '8px' }}>
        <label style={{ fontWeight: 'bold' }}>Adjust Bias (b): </label>
        <input type="range" min="-10" max="10" step="0.5" value={b} onChange={handleBiasChange} />
